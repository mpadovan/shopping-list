...
/**
 * Listener that handles the creation of the upload folder located in the
 * CATALINA_BASE directory. The directory and/or its subdirectories are newly
 * created if and only if they do not already exist.
 *
...
public class DirectoryManager implements ServletContextListener {
	...
	/**
	 * Creates the directories at application startup. If the outermost
	 * directory exists ($CATALINA_BASE/ShoppingList/Uploads), it assumes that
	 * also its sub-directories exist, otherwise it creates the whole nested
	 * structure. Eventually it sets the uploads folder path and its innermost
	 * directories (allowing for structural revision) as context attributes. One
	 * may safely assume that the path string set by this method is always
	 * terminated by <code>File.separator</code>.
	 *
	 * @param sce
	 */
	@Override
